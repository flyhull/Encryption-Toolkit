...
{
    [TestClass]
    public sealed class FastPositivePadBasedTests
    {
        [TestMethod]
        public void BytesToPadEncryptedBytesToBytes()
        {
            ...
            byte[] plaintext = TestingSupport.GetRandomNumberOfRandomBytes(1, PadBasedCryptionLimits.MaximumPlaintextBytes / 4);
            byte[] cryptionPad = TestingSupport.GetRandomNumberOfRandomBytes(PadBasedCryptionLimits.MinimumCryptionPadBytes, PadBasedCryptionLimits.MaximumCryptionPadBytes / 4);
            byte[] passphrase = TestingSupport.GetRandomNumberOfRandomBytes(TimeBasedCryptionLimits.MinimumPassPhraseLength, TimeBasedCryptionLimits.MaximumPassPhraseLength / 4);
            ...
            if (secretDate.Valid)
            {
                ...
                if (intermediate.Worked && intermediate.WroteBytes)
                {
                    result = Pad_Based_Encryption.UseTimeAndPadToStatically.Decrypt(intermediate.Bytes, cryptionPad, passphrase, secretDate.TimeStampValue, DateTime.UtcNow,
                        2, 180, 0, memSize, passes, new ValidationSummary());
